...
public class PatientController  {
    ...
    public void initialize() {
        ...
        colHos.setCellValueFactory(new PropertyValueFactory<>("hospital"));
        ...
        tblPatient.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            enableExitForm(id,newValue.getHospital());

            btnSave.setText("UPDATE");
            System.out.println(id);
            try {
                setPatient(id,newValue.getHospital());
            } catch (SQLException e) {
                e.printStackTrace();
            }
        });
       ...
       colHosExit.setCellValueFactory(new PropertyValueFactory<>("hosExId"));
       ...
       loadExitData();
        ...
        tblExit.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            System.out.println(id);
            try {
                setPatient(id,newValue.getHosExId());
            } catch (SQLException e) {
                e.printStackTrace();
            }
        });
    ...
    void btnSaveOnAction(ActionEvent event) throws SQLException {
        ...
        if(btnSave.getText().equalsIgnoreCase("Save")){
           //connection.setAutoCommit(false);
            ...
            try {
                ...
                if(i>0){
                    System.out.println("run");
                    PreparedStatement pst=connection.prepareStatement("insert into patient values(?,?,?,?)");
                    pst.setString(1,id);
                    pst.setString(2,hos);
                    pst.setDate(3,date);
        ...
        //Updating
            ...
            if(i>0){
                PreparedStatement pst=connection.prepareStatement("update patient set hospital=?,date=?,reason=? where id=?");
                pst.setString(1,hos);
                pst.setDate(2,date);
    ...
    private void loadPatient() throws SQLException {
        ...
        PreparedStatement pst= DBConnection.getInstance().getConnection().prepareStatement("select * FROM people pe inner join patient p on pe.id=p.id");
        ...
        while (rst.next()){
            ...
            PatientTM patientTM=new PatientTM(id,nm,city,dis,hos,pro,nic,date,ref,del);
    ...
    private void reset(){
       ...
       //txtId.setText("");
        ...
        cmbBlood.setValue("");
        dateDate.setValue(java.time.LocalDate.now());
        btnExit.setText("Exit");
        btnSave.setText("SAVE");
        btnCovid.setDisable(true);
    }

    private void setPatient(String id,String hosid) throws SQLException {

        System.out.println(id);

        PreparedStatement pst= DBConnection.getInstance().getConnection().prepareStatement("select * FROM people pe inner join patient p on pe.id=p.id where pe.id=? AND p.hospital=?");
        pst.setString(1,id);
        ...
        rst.next();
            ...
            txtReason.setText(rst.getString(14));
    ...
    private void loadHospitals(){
        ...
        try {
            PreparedStatement preparedStatement=connection.prepareStatement("select id from hospital");
    ...
    private void btnDelOnAction(String id){
        ...
        if (buttonType.get() == ButtonType.YES) {
            ...
            try {
                PreparedStatement pstm = DBConnection.getInstance().getConnection().prepareStatement("DELETE FROM patient WHERE id=?");
                pstm.setObject(1, id);
    ...
    public void iconHome(MouseEvent mouseEvent) throws IOException {

        //Navigating Based on Access privileges

        Stage stage1 = (Stage) this.ancPatient.getScene().getWindow();
        ...
        if (title.equalsIgnoreCase("admin")){
            ...
            Stage stage= (Stage) this.ancPatient.getScene().getWindow();
        ...
        if(btnExit.getText().equalsIgnoreCase("Exit")){
            try {
                ...
                if(i>0){
                    ...
                    PreparedStatement pst1=connection.prepareStatement("insert into hospital_exit values(?,?,?,?,?,?)");
                    pst1.setString(1,id);
